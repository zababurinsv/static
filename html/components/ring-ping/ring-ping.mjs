import webSocket from '/static/html/components/component_modules/webSocket/webSocketTon.mjs'
import config from '/static/html/components/component_modules/config/index.mjs'
customElements.define('ring-ping',
    class extends HTMLElement {
        ...
        constructor () {
            ...
            async function modules (obj) {
                document.addEventListener('ringPing',async ()=>{
                    let socket = {}
                    // console.assert(false, config['ton']['web']['wss'])
                    socket = new (await webSocket({name:'ton'}))['class'](config['ton']['web']['wss'])
                    let sseSource = new EventSource(config['ton']['web']['sse']);
                    sseSource.addEventListener('ping', (e) => {
                        // const messageData = JSON.parse(e.data);
                        // if(messageData['version']){
                        //     obj['this']['shadowRoot'].querySelector('#connect').insertAdjacentHTML('beforeend',`
                    // <div class="item">
