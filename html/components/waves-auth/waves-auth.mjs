import index  from '/static/html/react/waves-auth/index.mjs'
      ...
      constructor () {
        ...
        async function modules (obj) {
            bundle['default'](obj,null, async function (error, config) {
              ...
              let req = {}
              req = await Authtorization()

              new req['class']['network']({pid:'test1'})
              new req['class']['network']({pid:'test2'})
              new req['class']['local']()
              let inter =  new req['class']['interface']({pid:'waves-auth'})
              // let namespace =  req['class']['namespace']({pid:'waves-auth'})
              new req['class']['network']({pid:'test3'})
              // assert.deepEqual();
              // assert.equal(namespace, namespace2, '@#@@@@@@');
              // await req['network']['process']({_:'helloWorld', hello: ''})
              // await req['network']['process']({_:'testRequest', movie: {
              //   id:1,
              //   fields:'name, year, directorId'
              // }})
              // await req['network']['process']({_:'director', director: {
              //     id:1,
              //     fields:`name, id, age, movies{ name, year }`
              //   }})

              new req['class']['network']({pid:'test4'})

              inter.action()
              // console.assert(false, inter)
              try{
                WavesKeeper.on("update",async (data) => {
                  ...
                  if(auth){
                    ...
                    if(config['isEmpty'](clientAccount)){
                      clientAccount = {}
