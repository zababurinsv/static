...
customElements.define('lacerta-feed',
  class extends HTMLElement {
    constructor () {
      ...
      function objectProperty (obj) {
        return new Promise(function (resolve, reject) {
          ...
          if (!obj.hasAttribute('parent')) {
            console.warn('нет родительского элемента parent')
          } else {
            black['parent'] = obj.getAttribute('parent')
          }
          ...
          if (verify['swap'] === true) {
            for (let key = 0; key < obj['this'].children.length; key++) {
              ...
              if (obj['this'].children[key].tagName.split('-').length === 1) {
                ...
                if (obj['getAttribute'](obj['this'].children[key], 'light', 'template') === true) {
                  ...
                  console.log('ssssssssssssss11111111')
                  scriptTemplate(obj['this'].children[key])

                  // if (obj['template-head'].length !== 0) {
                  //   for (let key = 0; key < obj['template-head'].length; key++) {
                  //     scriptVerify(obj)
                  //   }
                  // }
                  obj['template-light'].push(obj['this'].children[key])
                  ...
                  obj['this'].children[key].setAttribute('type', `${obj['this'].children[key].getAttribute('type')}-external`)
                  console.log('ssssssssssssss222222222')
                  scriptTemplate(obj['this'].children[key])
                  // if (obj['template-head'].length !== 0) {
                  //   for (let key = 0; key < obj['template-head'].length; key++) {
                  //     scriptVerify(obj)
                  //   }
                  // }
                  console.log('3333333333', obj['this'].children[key])
            ...
            for (let key = 0; key < obj['template'].children.length; key++) {
              ...
              if (obj['template'].children[key].tagName.split('-').length === 1) {
                ...
                if (obj['getAttribute'](obj['template'].children[key], 'light', 'template') === true) {
                  ...
                  //   }
                  // }
                  console.log('ssssssssssssss333333333')
                  ...
                  //   }
                  // }
                  console.log('ssssssssssssss4444444444')
                  scriptTemplate(obj['template'].children[key])
                  console.log('333333444443333', obj['this'].children[key])
            ...
            for (let key = 0; key < obj['this'].children.length; key++) {
              ...
              if (obj['this'].children[key].tagName.split('-').length === 1) {
                ...
                if (obj['getAttribute'](obj['this'].children[key], 'light', 'template') === true) {
                  console.log('ssssssssssssss555555555')
                  scriptTemplate(obj['this'].children[key])
                  // if (obj['template-head'].length !== 0) {
                  //   for (let key = 0; key < obj['template-head'].length; key++) {
                  //     scriptVerify(obj)
                  ...
                  // }
                  obj['template-shadow'].push(obj['this'].children[key])
                  console.log('33333344555554443333', obj['this'].children[key])
                } else {
                  console.log('ssssssssssssss666666666666')
                  scriptTemplate(obj['this'].children[key])
                  // if (obj['template-head'].length !== 0) {
                  //   for (let key = 0; key < obj['template-head'].length; key++) {
                  //     scriptVerify(obj)
            ...
            for (let key = 0; key < obj['template'].children.length; key++) {
              ...
              if (obj['template'].children[key].tagName.split('-').length === 1) {
                ...
                if (obj['template'].children[key].tagName === 'SCRIPT') {
                  obj['template-head'] = []
                  obj['template-head'].push(obj['template'].children[key])
                  colorLog(`<varan-rss>obj['template-head']</varan-rss>`, obj['template'].children[key])
                } else {
                  obj['template-shadow'].push(obj['template'].children[key])
                }
              } else {
                if (obj['getAttribute'](obj['template'].children[key], 'light', 'template') === true) {
                  console.log('ssssssssssssss7777777777777')
                  scriptTemplate(obj['template'].children[key])
                  // if (obj['template-head'].length !== 0) {
                  //   for (let key = 0; key < obj['template-head'].length; key++) {
                  //     scriptVerify(obj)
                  //   }
                  // }
                  console.log('333333445777777888887755554443333', obj['this'].children[key])
                  obj['template-shadow'].push(obj['template'].children[key])
                } else {
                  console.log('ssssssssssssss888888888888888')
                  scriptTemplate(obj['template'].children[key])
                  // if (obj['template-head'].length !== 0) {
                  //   for (let key = 0; key < obj['template-head'].length; key++) {
                  //     scriptVerify(obj)
                  ...
                  getTemplate(obj, obj['type-swap'], obj['type-external'])
                    .then((obj) => {
                      ...
                      if (obj['verify']['swap'] === true) {
                        ...
                        console.assert(true, obj['template-head'])
                        // if (obj['template-head'].length !== 0) {
                        //   for (let key = 0; key < obj['template-head'].length; key++) {
                        //     scriptVerify(obj)
                        //   }
        ...
        return new Promise(function (resolve, reject) {
          fetch(`/static/html/components/varan-slider/template/${obj['slot']}.html`)
            .then(function (response) {
              ...
              //       resolve(obj)
              //     } else {
              //       if (obj['verify']['sliderText'] === false) {
              //         pagination['init'](obj)
      ...
      function scriptVerify (obj) {
        return new Promise(function (resolve, reject) {
          const script = document.createElement('script')
          let verify = false
          for (let key = 0; key < document['head'].getElementsByTagName('script').length; key++) {
            if (typeof (document['head'].getElementsByTagName('script')[key]) === 'object') {
              if (document['head'].getElementsByTagName('script')[key].outerHTML.indexOf(`${obj['component']}-bundle`) !== -1) {
                verify = true
              }
            }
          }
          if (verify === true) {
            colorLog(`<varan-rss>script уже загружен</varan-rss>`, 'yellow')
          } else {
            script.src = `/static/html/components/component_modules/${obj['component']}/${obj['component']}-bundle.mjs`
            script.type = 'module'
      ...
      async function modules (obj) {
        ...
        if(feeds['mongo'].length === 0){
          pre.innerText = 'нет каналов'
            obj['this'].shadowRoot.appendChild(pre)
          }else{
          let css = `<?xml-stylesheet type="text/css" href="/static/html/components/lacerta-feed/shadow/lacerta-feed.css" ?>`
          style="word-wrap: break-word; white-space: pre-wrap;"

          pre.style.wordWrap
          pre.innerText = feeds['mongo'][0]['feed']['rss']
          pre.innerText = pre.innerText.replace(/<!\[CDATA\[(.*)\]\]>/g, "$1")
