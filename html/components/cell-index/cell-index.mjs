...
// import Tick from './module/lacerta-tick/lacerta-tick.mjs'
import object from '/static/html/components/component_modules/varan-object/varan-object.mjs'
...
obj['function']['create'] = function ($root) {
  return new Promise(function (resolve, reject) {
    ...
    var Gene = {
      ...
      LCS: function (a, b) {
        var m = a.length; var n = b.length; var C = []; var i; var j; var af = []; var bf = []
...
obj['function']['proxy'] = function (obj) {
  ...
  resolve(true)
    ...
    const CORS_PROXY = ''
    fetch(CORS_PROXY + 'http://localhost:23443/')
      ...
      console.log('noosphere.ru', doc)
        ...
        // eval('iframe.write(doc)')
        let url = encodeURIComponent(doc)
        // iframe.src = url
        // iframe.open()
        // iframe.write(doc)
        // iframe.close()
   // console.assert(false, url)
        ...
        // iframe.baseURI = 'http://svoe-mesto.com/'
        // console.assert(false, iframe)
        // let content = document['body'].querySelector('iframe')

        // let content = document['body'].querySelector('.jason')
        // for (let i = 0; i < content.children.length; i++) {
        //   if (content.children[i].tagName === 'IFRAME') {
        //     console.dir('ssssssssssssssssss222222222sssssssssssssssssssss', content.children[i])
        //     content.replaceChild(iframe, content.children[i])
      ...
      .then((obj) => {
        let addBlock = new CustomEvent('addBlock', { })
        document.dispatchEvent(addBlock)
        resolve(obj)
      })
      .catch(error => {
        return error
...
customElements.define('cell-index',
  class extends HTMLElement {
    constructor () {
      super();
      (async (obj)=>{

        var loadScriptAsync = function(uri, obj){
          return new Promise((resolve, reject) => {
            ...
            var firstScriptTag = obj['this'].getElementsByTagName('script')[0];
        ...
        this.createDocumentFragment = function () { return document.createDocumentFragment() }
        ...
        this.createElement = function (obj) { return document.createElement(obj) }
        this.NodeList = this.childNodes
        this.NodeList.prototype = Array.prototype
        this.HTMLElement = window.HTMLElement
        let style = document.createElement('style')
        style.innerHTML = `@import './static/html/components/cell-index/module/jason.css;'`
        this.appendChild(style)
        await loadScriptAsync(`./static/html/components/cell-index/module/jason.js`, obj)
        await loadScriptAsync(`./static/html/components/cell-index/module/dragula.js`, obj)
        await loadScriptAsync(`./static/html/components/cell-index/module/st.js`, obj)

        // https://ducknet.ru/
        //     'url':'http://svoe-mesto.com/'
        let url = `http://localhost:23443/`
        switch (obj['slot']) {
          case 'cell-wasm':
            url = `http://localhost:28443/`
            break
          case 'cell-main':
