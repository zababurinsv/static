    ...
    class extends HTMLElement {
      ...
      constructor () {
        ...
        let login = new (await Login({name:'telegram-login'}))['class']
          ...
          obj['this']['shadowRoot'].querySelector('#phone').addEventListener('input',async ()=>{
          ...
          obj['this']['shadowRoot'].querySelector('#next').addEventListener('click',async (e)=>{
            ...
            let phone = `${obj['this']['shadowRoot'].querySelector('#code').innerText}${obj['this']['shadowRoot'].querySelector('#phone').value}`
            // console.assert(false,  phone )
            ...
            `)
                ...
                <p>enter you phone number</p>`)
                  ...
                  obj['this']['shadowRoot'].querySelector('#next').innerText = 'PLEASE WAIT...'
                  obj['this']['shadowRoot'].querySelector('.message').innerText = ''
                  obj['this']['shadowRoot'].querySelector('.message').insertAdjacentHTML('afterbegin',`
                 <p>We have sent you an sms</p>
                 <p>with the code</p>`)
                  obj['this']['shadowRoot'].querySelector('.loader').style.display = 'flex'
                ...
                obj['this']['shadowRoot'].querySelector('#verification').addEventListener('input',async ()=>{
                  ...
                  if(obj['this']['shadowRoot'].querySelector('#verification').value.length === 0){
                    ...
                    if( obj['this']['shadowRoot'].querySelector('#verification').value.length === 5){
                      socket['socket'].send(JSON.stringify({type:'code',code:obj['this']['shadowRoot'].querySelector('#verification').value}));
                    }
               ...
               await login['templatePhoto'](obj)
                ...
                document.addEventListener('telegram-login-action',async (e)=>{
                  bio['photo'] = e['detail']['photo']['dataURL']
                  if(bio['name'] === undefined){
              ...
              obj['this']['shadowRoot'].querySelector('#code').style.borderColor = 'red'
              obj['this']['shadowRoot'].querySelector('#phone').style.borderColor = 'red'
            }
            // socket['socket'].send(JSON.stringify({type:'phone',phone:phone}));
          })
          document.addEventListener('telegram-login-chat-page', async (object)=>{
            if(object['detail']['data']['getAuthorizationState'] === 'authorizationStateReady'){
            }else{
              // obj['this']['shadowRoot'].querySelector('select').addEventListener('change',async (e)=>{
              // [e.target.options[e.target.selectedIndex].value]
              // console.assert(false, await login['code'](e.target.value))
              // })
              //
              /**
               * verification
