import Stackexchange from '/static/html/components/component_modules/stackexchange/stackexchange.mjs'
...
customElements.define('stack-exchange',
    class extends HTMLElement {
      ...
      constructor () {
        ...
        async function modules (obj) {
          ...
          let acces = location.hash
          removeHash()
          console.log('~~~~~~~~~location.hash~~~~~~~~~', acces)
          let stackexchange = new (await Stackexchange())['class']()
          // stackexchange.init()
          obj['this']['shadowRoot'].querySelector('button.authtorization').addEventListener('click',async ()=>{

            // stackexchange.auth()

              try{
              let response =  await fetch(`${await Config('stack-exchange', 'heroku')}auth/stackexchange`, {
                method: "GET",
                mode: 'no-cors'
              })
