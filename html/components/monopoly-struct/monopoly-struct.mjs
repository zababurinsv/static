...
import Materia from '/static/html/components/component_modules/materia/materia.mjs'
import Modal from '/static/html/components/component_modules/modal/modal.mjs'
customElements.define('monopoly-struct',
    class extends HTMLElement {
      ...
      constructor () {
        ...
        async function modules (obj) {
          ...
          let interfaceGame = new (await Materia())['class']['interface']()
          let classGame = new (await Materia())['class']['local']()
          Modal({ id:'universe' }).then(async (data)=>{

            obj.this.shadowRoot.querySelector('#struct').insertAdjacentHTML('afterbegin', data)
            obj['this'].shadowRoot.querySelector('#close').addEventListener('click',async (event)=>{
              obj['this'].shadowRoot.querySelector('#id01').style.display='none'
            })
            obj['this'].shadowRoot.querySelector('#addWorld').addEventListener('click',async (event)=>{
              obj['this'].shadowRoot.querySelector('#id01').style.display='block'
              obj['this'].shadowRoot.querySelector('#id01').style.zIndex='999999999'
            })


            obj['this'].shadowRoot.querySelector('button.next').addEventListener('click',async (event)=>{
              document.dispatchEvent(new CustomEvent('first.step', {
                detail: {
                  data:'first.step'
                }
              }))
            })
            obj['this'].shadowRoot.querySelector('button.previous').addEventListener('click',async (event)=>{
              document.dispatchEvent(new CustomEvent('previous.step', {
                detail: {
                  data:'previous.step'
                }
              }))
          ...
          document.addEventListener('first.step',async (event)=>{

            let node = obj['this'].shadowRoot.querySelector('div.w3-container')
            let label =  node.querySelectorAll('label')
            // for(let key in )
            for(let i =0; i < label.length;i++){

              console.log('~~~~~~~~~~~~~~~~~~~>>>>>>', label[i].querySelector('input').value)
            }
            node.innerHTML = ''
            obj['this'].shadowRoot.querySelector('button.next').style.display = 'none'
            obj['this'].shadowRoot.querySelector('button.previous').style.display = 'flex'
            obj['this'].shadowRoot.querySelector('button.create').style.display = 'flex'

            let bio = {}
            bio['first_name'] = undefined
            bio['last_name'] = undefined
            bio['photo'] = undefined
            node.insertAdjacentHTML('afterbegin', await Modal({ id:'universe.avatar',obj:obj }))

            obj['this'].shadowRoot.querySelector('#image').onchange = function (event) {
              console.log('~~~~~~~~~~~~~3333~~~')
              obj['this'].shadowRoot.querySelector('div.w3-container').insertAdjacentHTML('afterbegin', `<div><slot name="varan-crop"></slot></div>`)
              obj['this'].shadowRoot.querySelector('#image').style.display = 'none'
              document.dispatchEvent( new CustomEvent('universe', {
                detail: {
                  id: obj['component'],
                  slot: obj['slot'],
          ...
          document.addEventListener('previous.step',async (event)=>{
            obj['this'].shadowRoot.querySelector('div.w3-container').innerHTML = ''
            obj['this'].shadowRoot.querySelector('div.w3-container').insertAdjacentHTML('afterbegin', await Modal({ id:'universe.first' }))
            obj['this'].shadowRoot.querySelector('button.next').style.display = 'flex'
            obj['this'].shadowRoot.querySelector('button.previous').style.display = 'none'
            obj['this'].shadowRoot.querySelector('button.create').style.display = 'none'
          })
          // await Login['templatePhoto'](obj)
          document.addEventListener('monopoly-struct',async (event)=>{

            obj['this'].shadowRoot.querySelector('div.w3-container').insertAdjacentHTML('afterbegin', await Modal({ id:'universe.photo', data:event.detail.photo.dataURL}))
          ...
          ///////////////////////////////////////////////////////////
          console.log('~~class~~',classGame)
          console.log('~~interfaceGame~~',interfaceGame)
        }
      }
      connectedCallback() {
