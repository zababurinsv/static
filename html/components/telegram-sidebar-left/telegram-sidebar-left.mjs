...
import sidebarLeft from '/static/html/components/component_modules/sidebarLeft/sidebarLeft.mjs'
import Config from '/static/html/components/component_modules/config/config.mjs'
customElements.define('telegram-sidebar-left',
      ...
      async function modules (obj) {
        let sidebar = new (await sidebarLeft({name:'telegram-sidebarLeft'}))['class']
        let socket = new (await webSocket({name:'telegram-login'}))['class'](await Config('telegram', 'localWss'))
        async function listener(e){
                obj['this']['shadowRoot'].querySelectorAll('.item')
                for(let i = 0; i < obj['this']['shadowRoot'].querySelectorAll('.item').length; i++){
                  obj['this']['shadowRoot'].querySelectorAll('.item')[i].style.background = 'white'
                }
                let item = e.target
                let verify = true
                while(verify){
          ...
          if(item.id.indexOf('u') > -1){
            id = parseInt(item.id.replace('u',''), 10)
          }else if(item.id.indexOf('s') > -1){
            id = parseInt(item.id.replace('s',''), 10)
          }
          item.style.background = '#f4f4f5'
          socket['socket'].send(JSON.stringify({type:'createPrivateChat', user_id:id}))
        }

        document.addEventListener('user',async (e)=>{
          if(obj['this']['shadowRoot'].querySelector(`#u${e['detail']['id']}`) !== null){
            obj['this']['shadowRoot'].querySelector(`#u${e['detail']['id']}`).remove()
          }
            sessionStorage.setItem('first_name', e['detail']['first_name']);
            sessionStorage.setItem('last_name', e['detail']['last_name']);
            sessionStorage.setItem('user_id', e['detail']['id']);
            obj['this']['shadowRoot'].querySelector('#items').insertAdjacentHTML('beforeend', `
              <div class="item" id="u${e['detail']['id']}">
                <div class="avatar"><img style="box-sizing: border-box; padding: .5vw;" src="/static/html/components/telegram-sidebar-left/images/my.png"/></div>
                <div class="info">
                    <div class="name">${e['detail']['first_name']} ${e['detail']['last_name']}</div>
                    <div class="status"> ${e['detail']['status']}</div>
                    <div class="id" style="display: none">${e['detail']['id']}</div>
                    <div class="private" style="display: none">${e['detail']['id']}</div>
                </div>
            </div>
          `)
          obj['this']['shadowRoot'].querySelector(`#u${e['detail']['id']}`).addEventListener('click', listener)
        })
        document.addEventListener('updateUser',async (e)=> {
          if(obj['this']['shadowRoot'].querySelector(`#u${e['detail']['id']}`) === null){
            if(e['detail']['status'] === 'userStatusOffline'){}else{
              obj['this']['shadowRoot'].querySelector('#items').insertAdjacentHTML('beforeend',`
              <div class="item" id="u${e['detail']['id']}">
                <div class="avatar"><img style="box-sizing: border-box; padding: .5vw;" src="/static/images/no_image.jpg"/></div>
                <div class="info">
                    <div class="name">${e['detail']['first_name']} ${e['detail']['last_name']}</div>
                    <div class="status"> ${e['detail']['status']}</div>
                    <div class="id" style="display: none">${e['detail']['id']}</div>
          ...
          `)
            ...
            if(e['detail']['status'] === 'userStatusOffline'){
              obj['this']['shadowRoot'].querySelector(`#u${e['detail']['id']}`).remove()
            }
          }
        })

        document.addEventListener('updateSupergroup',async (e)=> {
          if(obj['this']['shadowRoot'].querySelector(`#s${e['detail']['id']}`) === null){
            obj['this']['shadowRoot'].querySelector('#items').insertAdjacentHTML('beforeend',`
              <div class="item" id="s${e['detail']['data']['supergroup']['id']}">
                <div class="avatar"><img style="box-sizing: border-box; padding: .5vw;" src="/static/images/no_image.jpg"/></div>
                <div class="info">
                    <div class="name">${e['detail']['data']['supergroup']['username']}</div>
                    <div class="date style="display: none">${e['detail']['data']['supergroup']['member_count']}</div>
                </div>
            </div>
          `)
            obj['this']['shadowRoot'].querySelector(`#s${e['detail']['data']['supergroup']['id']}`).addEventListener('click', listener)
          }
        })
        document.addEventListener('telegram-login-chat', async (e)=>{
          let name = `${e['detail']['data']['first_name']} ${e['detail']['data']['last_name']}`
          obj['this']['shadowRoot'].querySelector('.name').innerText = ''
          obj['this']['shadowRoot'].querySelector('.name').innerText = name
          obj['this']['shadowRoot'].querySelector('.id').innerHTML = ''
          obj['this']['shadowRoot'].querySelector('.id').innerHTML = e['detail']['data']['id']
          obj['this']['shadowRoot'].querySelector('.private').innerHTML = e['detail']['data']['id']

        })
          document.addEventListener('telegram-private-chat',async (e)=>{
              obj['this']['shadowRoot'].querySelector('.private').innerText = ''
              obj['this']['shadowRoot'].querySelector('.private').innerText = name
              obj['this']['shadowRoot'].querySelector('.private').innerText = e['detail']['id']
          })
        //
        // console.assert(false )

            /*
            if(request['status']['message'] === 'PHONE_CODE_INVALID' || request['status']['code'] === 429){

              obj['this'].parentNode.querySelector('telegram-chat').style.display = 'none'
              obj['this'].parentNode.querySelector('telegram-message').style.display = 'none'

              for(let i =0; i < document['body']['shadowRoot'].querySelectorAll('nav').length;i++){

                // console.assert(false, document['body']['shadowRoot'].querySelectorAll('nav')[i])
                document['body']['shadowRoot'].querySelectorAll('nav')[i].style.display = 'none'
              }
            }else{
              let conteiner = obj['this']['shadowRoot'].querySelector('#items')

              document.addEventListener('telegram-leftPanel-rightPanel', async (e)=>{
                obj['this']['shadowRoot'].querySelector('.id').innerText = e['detail']['data']['getMe']['id']

              })


              socket['socket'].send(JSON.stringify({type:'getMe'}))

              // conteiner.innerHTML = ''
              for(let i=0;i< request.length;i++){
                request[i]['status']['_'] = request[i]['status']['_'].replace('userStatus','')
                conteiner.insertAdjacentHTML('beforeend',`
                       <div class="item">
                            <div class="avatar"><img src="/static/images/no_image.jpg"/></div>
                            <div class="info">
                                <div class="name">${request[i]['first_name']} ${request[i]['last_name']}</div>
                                <div class="status">${request[i]['status']['_']}</div>
                                <div class="id" style="display: none">${request[i]['id']}</div>
                            </div>
                        </div>
                      `)
                // console.log('~~~~~~~!!!!~~~~~~~~~~~~~~',request[i]['status']['_'])
              }
              let items = obj['this']['shadowRoot'].querySelector('#items').children
              ...
              for(let i=0;i< items.length;i++){
                ...
                items[i].addEventListener('click',async (e)=>{
                  ...
                  let itemId ={}
                  itemId['type'] = 'getUserFullInfo'
                  itemId['user_id'] = object.querySelector('.id').innerText
                  socket['socket'].send(JSON.stringify({
                    type:'getUserFullInfo',
                    user_id: object.querySelector('.id').innerText
                  }))
                  // let item = new CustomEvent('telegram-rigthSidebar-chat', {
                  //   detail: {
                  //     id: object.querySelector('.id').innerText,
                  //     name:object.querySelector('.name').innerText,
                  //     photo:object.querySelector('img').src
                  //   }
                  // })
              ...
              document.addEventListener('telegram-sidebar-left',async (obj)=>{

                console.assert(false, obj['detail'])
