...
customElements.define('web3-authorization-vk',
    class extends HTMLElement {
      ...
      constructor () {
        ...
        async function modules (obj) {
          ...
          obj['this']['shadowRoot'].querySelector('#authtorization').addEventListener('click',async ()=>{
            ...
            // 'height=50vw,width=45vw'
           // let newWin = window.open(`${await Config('vk', 'web')}auth/vkontakte`,'authtorization',`height=${scrollWidth/3},width=${scrollWidth/1.5},scrollbars=no,toolbar=no,menubar=no,status=no,resizable=no,scrollbars=no,location=no,top=${scrollWidth/2-((scrollWidth/1.5)/2)},left=${scrollWidth/2-((scrollWidth/1.8)/2)}`);
           ...
           //  }
            ...
            // newWin.onclick = function (eee) {
            //   console.log('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
            // }
            // newWin.document.body.onclick = function (e) {
            //
            // }

            // console.log('~~~~~~~~~body~~~~~~', newwindow)
            // newwindow.addEventListener('load', async (e)=>{
            //
            ...
            // }, false);
            try{
              let response =  await fetch(`${await Config('vk', 'web')}auth/vkontakte`, {
                method: "GET",
                mode: 'no-cors'
              })
              if (response.ok) {
