...
customElements.define('lacerta-news',
    class extends HTMLElement {
      ...
      constructor () {
        ...
        // console.assert(false, obj['this'])
            // obj['this'].shadowRoot.querySelector('.graph').style.display = 'none'
          }
          ...
          if(verify){
            await store({
              ...
              this:obj['this'],
              obj: obj,
              img: obj['this'].shadowRoot.querySelector('.gallery'),
              slot:obj.slot,
              type:'obj'
            }, 'set', 'type')
            ...
            if(feeds === undefined || feeds === null || feeds.length === 0){
              ...
              // console.assert(false, feeds)
                ...
                obj['this'].shadowRoot.querySelector('#news').innerHTML = ''
               for(let m = 0; m < feeds.length; m ++){
                 let outTemplate =  await templateItem({
                   input:'lacerta-news',
                   data:feeds[m],
                   type:'news'
                 },'create', 'type')
                   obj['this'].shadowRoot.querySelector('#news').insertAdjacentHTML('beforeend',outTemplate )
                  let item = obj['this'].shadowRoot.querySelector(`.item_${Date.parse(feeds[m]['isoDate'])}`)
                  let parent = item.querySelector('.date').parentNode
                  let timestamp = parseInt(Date.parse(feeds[m]['isoDate']), 10)
                 ...
                 item.addEventListener('click', (event) => {
                   console.log('1', event.target.tagName)
                   console.log('2', event.target.parentNode.tagName)
                   console.log('3', event.target.parentNode.parentNode.tagName)
