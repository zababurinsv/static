...
import * as IMask from '/static/html/components/component_modules/imask/esm/index.js'
    ...
    class extends HTMLElement {
      ...
      constructor () {
        ...
        function scriptTemplate (obj, parent) {
          return new Promise(function (resolve, reject) {
            let verify = false
            for (let i = 0; i < document.querySelectorAll(obj.tagName.toLowerCase()).length; i++) {
              let temp = document.querySelectorAll(obj.tagName.toLowerCase())[i]

              if(temp.shadowRoot === null){
                if(temp.querySelectorAll('script').length > 0){
                  for(let j =0; j < temp.shadowRoot.querySelectorAll('script').length; j++){

                    if (temp.querySelectorAll('script')[j].src.indexOf(obj.tagName.toLowerCase()) !== -1) {
                      verify = true
                    }
                  }
                }
              }else{
                if(temp.shadowRoot.querySelectorAll('script').length > 0){
                  for(let j =0; j < temp.shadowRoot.querySelectorAll('script').length; j++){

                    if (temp.shadowRoot.querySelectorAll('script')[j].src.indexOf(obj.tagName.toLowerCase()) !== -1) {
                      verify = true
                    }
                  }
                }
            ...
            switch (obj.tagName.split('-').length) {
              case 2:
                for (let i = 0; i < document.querySelectorAll('script').length; i++) {

                  switch (document.querySelectorAll('script')[i].src.split('-').length) {
                    case 5:
                      break
                    case 3:
                    ...
                    case 4:
                      ...
                      break
                    default:
                      // console.assert(false, 'неизвестный тип', document.querySelectorAll('script')[i].src.split('-'))
                      break
                  }
                }
                break
              case 3:
                switch (document.querySelectorAll('script')[i].src.split('-')) {
                  case 5:
                    if (document.querySelectorAll('script')[i].src.indexOf(obj.tagName.toLowerCase()) !== -1) {
                      verify = true
                    }
                ...
                break
              default:
                    console.assert(false, 'этот тип не потдерживается слишком длинное название', obj.tagName.split('-'))
                break
            }
            if (verify === true) {
        ...
        async function modules (obj) {
          ...
          const cardnumber = obj['this'].shadowRoot.querySelector('#number');
          const expirationdate = obj['this'].shadowRoot.querySelector('#expiration');
          const expirationyear = obj['this'].shadowRoot.querySelector('#expiration-year');
          const securitycode = obj['this'].shadowRoot.querySelector('#code');
          const output = obj['this'].shadowRoot.querySelector('#output');
          const ccicon = obj['this'].shadowRoot.querySelector('#ccicon');
          const ccsingle = obj['this'].shadowRoot.querySelector('#ccsingle');
          const generatecard = obj['this'].shadowRoot.querySelector('#generatecard');

          let cctype = null;

          let cardnumber_mask = new IMask.InputMask(cardnumber, {
            mask: [
              {
          ...
          let expirationdate_mask = new IMask.InputMask(expirationdate, {
            mask: 'MM',
            blocks: {
              MM: {
          ...
          let expirationYear_mask = new IMask.InputMask(expirationyear, {
            mask: 'YY',
            blocks: {
              YY: {
                mask: '00',
          ...
          let securitycode_mask = new IMask.InputMask(securitycode, {
            mask: '0000',
          });
          ...
          const swapColor = function (basecolor) {
            ...
            obj['this'].shadowRoot.querySelectorAll('.lightcolor')
                .forEach(function (input) {
                  input.setAttribute('class', '');
                  input.setAttribute('class', 'lightcolor ' + basecolor);
                });
            obj['this'].shadowRoot.querySelectorAll('.darkcolor')
                .forEach(function (input) {
                  input.setAttribute('class', '');
...
// CREDIT CARD IMAGE JS
          obj['this'].shadowRoot.querySelector('.preload').classList.remove('preload');
          obj['this'].shadowRoot.querySelector('.creditcard').addEventListener('click', function () {
...
//On Input Change Events
          name.addEventListener('input', function () {
            if (name.value.length == 0) {
              obj['this'].shadowRoot.getElementById('svgname').innerHTML = 'John Doe';
              obj['this'].shadowRoot.getElementById('svgnameback').innerHTML = 'John Doe';
            } else {
              obj['this'].shadowRoot.getElementById('svgname').innerHTML = this.value;
              obj['this'].shadowRoot.getElementById('svgnameback').innerHTML = this.value;
            }
          });

          cardnumber_mask.on('accept', function () {
            if (cardnumber_mask.value.length === 0) {
              obj['this'].shadowRoot.getElementById('svgnumber').innerHTML = '0123 4567 8910 1112';
            } else {
              obj['this'].shadowRoot.getElementById('svgnumber').innerHTML = cardnumber_mask.value;
            }
          });
          ...
          expirationdate_mask.on('accept', function () {
            ...
            if(expirationYear_mask.value.length !== 0) {
                year = (expirationYear_mask.value.length === 1)
                  ? `0${expirationYear_mask.value}`
                  : expirationYear_mask.value
            }

            if (expirationdate_mask.value.length !== 0) {
              date = (expirationdate_mask.value.length === 1)
                ? `0${expirationdate_mask.value}`
                : expirationdate_mask.value
            }

            obj['this'].shadowRoot.getElementById('svgexpire').innerHTML = `${ date}/${year}`;
          });
          ...
          securitycode_mask.on('accept', function () {
            if (securitycode_mask.value.length == 0) {
              obj['this'].shadowRoot.getElementById('svgsecurity').innerHTML = '985';
            } else {
              obj['this'].shadowRoot.getElementById('svgsecurity').innerHTML = securitycode_mask.value;
            }
          });

//On Focus Events
          name.addEventListener('focus', function () {
            obj['this'].shadowRoot.querySelector('.creditcard').classList.remove('flipped');
          });
