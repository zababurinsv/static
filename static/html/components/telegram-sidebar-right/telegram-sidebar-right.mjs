...
import sidebarRight from '/static/html/components/component_modules/sidebarRight/sidebarRight.mjs'

customElements.define('telegram-sidebar-right',
      ...
      async function modules (obj) {
        let sidebar = new (await sidebarRight({name:'telegram-sidebarRight'}))['class']

        let me = {}

        document.addEventListener('telegram-login-chat', async (e)=>{
          sidebar['setUser'](obj, e['detail']['data'])
        })
        document.addEventListener('PrivateChat',async (e)=>{

          // console.assert(false, e['detail']['data']['PrivateChat'])
          sidebar['rightPanel'](obj, e['detail']['data']['PrivateChat']['RightPanel'])
          // console.assert(false, e['detail']['data']['RightPanel'])
        })
        document.addEventListener('telegram-RightPanel', async (e)=>{

          // console.log(e['detail']['data']['RightPanel'])
          let Info = {}
          for(let key in e['detail']['data']['RightPanel'] ){
            switch (key) {
              case 'getUserFullInfo':

                Info['bio'] = e['detail']['data']['RightPanel'][key]['bio']
                // console.assert(false, e['detail']['data']['RightPanel'])
                break
              case 'getUser':
                Info['fullName'] = `${e['detail']['data']['RightPanel'][key]['first_name']} ${e['detail']['data']['RightPanel'][key]['last_name']}`
                Info['status'] = e['detail']['data']['RightPanel'][key]['status']['_']
                Info['username'] = e['detail']['data']['RightPanel'][key]['username']
                Info['phoneNumber'] = `+${e['detail']['data']['RightPanel'][key]['phone_number']}`
                console.log('getUserFullInfo', e['detail']['data']['RightPanel'][key])
                // console.assert(false, e['detail']['data']['RightPanel'])
                break
              case 'getUserProfilePhotos':
                  for(let i =0; i < e['detail']['data']['RightPanel'][key]['photos'].length;i++){
                    Info['photo'] = e['detail']['data']['RightPanel'][key]['photos'][i]['sizes'][1]['photo']['remote']['id']
                    console.log('~~~~~~',e['detail']['data']['RightPanel'][key]['photos'][i]['sizes'][1]['photo']['remote']['id'])
                  }
                break
              default:
            console.warn('необрабатываемое событие', key )
                break
            }
          }
          ...
          obj['this']['shadowRoot'].querySelector('#preview').insertAdjacentHTML('afterbegin', `
            ...
            <div id="fullName">${Info['fullName']}</div>
