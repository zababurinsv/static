import graph from '/static/html/components/component_modules/graph/osc.mjs'
customElements.define('audio-oscillator',
    class extends HTMLElement {
      ...
      constructor () {
        ...
        async function modules (obj) {
          ...
          let app = new (await graph({name:'audio-oscilator'}))['class'](obj)
          await app.init(obj)
          // console.assert(false, app['self'])
          // let socket = new (await webSocket({name:'telegram-login'}))['class']

          let range = obj['this']['shadowRoot'].querySelector('#range');
          let AudioContext = window.AudioContext || window.webkitAudioContext;
          let context = new AudioContext();
          await  context.audioWorklet.addModule('/static/html/components/component_modules/oscillator/processor.mjs')
          ...
          let oscl = {}
          async function osc(bool){
            if(bool.target.value === undefined) bool = "true";
            if(bool.target.value === "true") {
              oscl = new OscillatorNode(context);
              let analyser = context.createAnalyser();
              oscl.connect(WorkletNode).connect(analyser).connect(context.destination);
              ...
              let canvas = obj['this']['shadowRoot'].querySelector("#oscilloscope");
              ...
              oscl.stop(context.currentTime);
            }
          }
          // obj['this']['shadowRoot'].querySelector('#play').addEventListener('click',  osc)
          // obj['this']['shadowRoot'].querySelector('#stop').addEventListener('click',  osc)
        }
      }
      connectedCallback() {
