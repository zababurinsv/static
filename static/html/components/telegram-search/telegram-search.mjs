...
customElements.define('telegram-search',
    class extends HTMLElement {
      ...
      constructor () {
        ...
        async function modules (obj) {
          ...
          obj['this']['shadowRoot'].querySelector('.dropdown').insertAdjacentElement('beforeend',login.template)
          // obj['this'].shadowRoot.querySelector('#myInput').addEventListener('input',async ()=>{
          //   console.assert(false)
          // })

          obj['this']['shadowRoot'].querySelectorAll('.country').forEach( (element) => {
            element.addEventListener('click', async (e)=>{

              obj['this']['shadowRoot'].querySelector('.dropbtn').id = 'set'
              // obj['this'].shadowRoot.querySelector('.dropbtn').disable = 'true'
              let country = {}
              if( e.target.className === 'country'){
                country = e.target
              }else{
                country = e.target.parentNode
              }
              obj['this'].shadowRoot.querySelector('.dropbtn').style.cursor = 'pointer'
              obj['this'].shadowRoot.querySelector('.dropbtn').type = 'button'
              obj['this'].shadowRoot.querySelector('.dropbtn').value = country.querySelector('.name').innerText
              obj['this'].shadowRoot.querySelector('.down').src = '/static/html/components/telegram-search/images/down_svg.svg'

              let telegramLogin = new CustomEvent('telegram-login', {
                detail: {
                  code: country.querySelector('.code').innerText,
                  type:'login'
                }
              })
              document.dispatchEvent(telegramLogin)
              obj['this'].shadowRoot.querySelector('#myDropdown').classList = 'dropdown-content';

            })
          })
          obj['this'].shadowRoot.querySelector('.dropdown').addEventListener('click',sFilter)

         async function sFilter(e) {
            if(obj['this']['shadowRoot'].querySelector('.dropbtn').id === 'set'){
              obj['this']['shadowRoot'].querySelector('.dropbtn').id = ''
            }else{
              obj['this'].shadowRoot.querySelector('.dropbtn').style.cursor = 'text'
              obj['this'].shadowRoot.querySelector('.dropbtn').value = ""
              obj['this'].shadowRoot.querySelector('.dropbtn').type = 'text'
              obj['this'].shadowRoot.querySelector('.down').src = '/static/html/components/telegram-search/images/up_svg.svg'
              obj['this'].shadowRoot.querySelector('#myDropdown').classList.toggle("show");

              if(obj['this'].shadowRoot.querySelector('#myDropdown').classList.length === 1){
                obj['this'].shadowRoot.querySelector('.dropbtn').type = 'button'
                obj['this'].shadowRoot.querySelector('.dropbtn').value = 'Country'
                obj['this'].shadowRoot.querySelector('.down').src = '/static/html/components/telegram-search/images/down_svg.svg'
                obj['this'].shadowRoot.querySelector('.dropbtn').style.cursor = 'pointer'
              }
              let input, filter, ul, li, a, i;
              obj['this']['shadowRoot'].querySelector('.dropbtn').addEventListener('input',async (e)=>{
                filter = e.target.value.toUpperCase();
                let div = obj['this']['shadowRoot'].querySelector('#myDropdown');
                for (i = 0; i < div.children.length; i++) {
                  if (div.children[i].querySelector('.name').innerHTML.toUpperCase().indexOf(filter) > -1) {
                    div.children[i].style.display = "flex";
                  } else {
