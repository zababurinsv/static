...
customElements.define('telegram-sidebar-left-head',
  class extends HTMLElement {
    static get observedAttributes () {
      return ['elWidth', 'elHeight', 'cursorTop', 'cursorLeft']
    }
    constructor () {
      super()
      let white = []
      ...
      async function module (obj) {
        let socket = new (await webSocket({name:'telegram-login'}))['class'](await Config('telegram', 'localWss'))
        obj['this']['shadowRoot'].querySelector('.menu').addEventListener('click',async (e)=>{

          if(obj['this']['shadowRoot'].querySelector('.menu').id === 'view'){
            obj['this']['shadowRoot'].querySelector('.menu').id = ''
            obj['this']['shadowRoot'].querySelector('#alert').remove()
          }else{
            obj['this']['shadowRoot'].querySelector('.menu').id = 'view'
            obj['this']['shadowRoot'].querySelector('.menu').insertAdjacentHTML('afterbegin',`
          <div id="alert">
          ...
          `)
            ...
            obj['this']['shadowRoot'].querySelector('#logOut').addEventListener('click',async (e)=>{

              socket['socket'].send(JSON.stringify({type:'logOut'}))
            })
            obj['this']['shadowRoot'].querySelector('#delete').addEventListener('click',async (e)=>{

              socket['socket'].send(JSON.stringify({type:'deleteAccount',reason:'test' }))
            })
          }
        })
      }
