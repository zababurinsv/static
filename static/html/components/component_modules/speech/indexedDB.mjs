            ...
            db['request'].onupgradeneeded = function(event) {
                ...
                switch (obj['name']) {
                    case'flatory':
                        ...
                        db['objectStore'] = db['db'].createObjectStore("cat_object", { keyPath: "id" });
                        db['objectStore'].createIndex("city", "city", { unique: false });
                        db['objectStore'].createIndex("class", "class", { unique: false });
                        db['objectStore'].createIndex("dateExperiences", "dateExperiences", { unique: false })
                        db['objectStore'].createIndex("description", "description", { unique: false })
                        db['objectStore'].createIndex("finish", "finish", { unique: false })
                        db['objectStore'].createIndex("geo", "geo", { unique: false })
                        db['objectStore'].createIndex("metro", "metro", { unique: false })
                        db['objectStore'].createIndex("name", "name", { unique: false })
                        db['objectStore'].createIndex("object", "object", { unique: false })
                        db['objectStore'].createIndex("plans", "plans", { unique: false })
                        db['objectStore'].createIndex("price", "price", { unique: false })
                        db['objectStore'].createIndex("tags", "tags", { unique: false })
                        ...
                        db['objectStore'].createIndex("type", "type", { unique: false })
                        if(obj['data']){
        ...
        return new Promise(async (resolve, reject) => {
            ...
            switch(obj['type']){
                case 'dictionary':
                    ...
                    break
                case 'id':
                (async (obj, type, rest)=>{ await connect({name:'flatory', version: 1 },async (db)=>{
                    // console.assert(false, obj['id'].length)
                    let object = []
                    if(obj['id'].length === 0 ){

                        db.transaction(['cat_object'], "readwrite").objectStore('cat_object').openCursor().onsuccess = (e)=>{
                            let cursor = e.target.result;
                            if (cursor) {
                                object.push(cursor['value'])
                                cursor.continue();
                        ...
                        db.transaction(['cat_object'], "readwrite").objectStore('cat_object').get(obj['id']).onsuccess = (e)=>{
                ...
                case 'getAll':
                    (async (obj, type, rest)=>{
                        ...
                        await connect({name:'flatory', version: 1 },async (db)=>{
                       let object = []
                ...
                case 'index':
                    (async (obj, type, rest)=>{ await connect({name:'flatory', version: 1 },async (db)=>{
                        let index =    db.transaction(['cat_object'], "readwrite").objectStore('cat_object').index(`${obj['index']}`)
                        let object = []
                        if(obj['index'] === 'price'){
            ...
            switch(obj['type']){
                case 'data':
                    (async (obj, type, rest)=>{
                        ...
                        await connect({name:'flatory', version: 1 },async (db)=>{
                            for (let i in data) {
                                db.transaction(['cat_object'], "readwrite").objectStore('cat_object').put(data[0]).onsuccess = (e)=>{
                                    if(data.length -1 == i){
                                        out(true)
                                    }
                                }
                ...
                case 'item':
                    (async (obj, type, rest)=>{
                        await connect({name:'flatory', version: 1 },async (db)=>{
                                db.transaction(['cat_object'], "readwrite").objectStore('cat_object').put(obj['value']).onsuccess = (e)=>{
                                    out(true)
                                }
                        })
                    })(obj, type, rest)
        ...
        return new Promise(async (resolve, reject) => {
            ...
            switch(obj['type']){
                case 'test':
                    (async (obj, type, rest)=>{
                        await connect({name:'flatory', version: 1 },async (db)=>{
                            (db.transaction(['cat_object'], "readwrite").objectStore('cat_object').delete('144')).onsuccess = (e)=>{
