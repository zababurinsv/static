...
customElements.define('varan-pictures',
      ...
      function objectProperty (obj) {
        return new Promise(function (resolve, reject) {
          ...
          if (!obj.hasAttribute('parent')) {
            if (!obj.hasAttribute('slot')) {
              console.assert(false, 'установите атрибут', obj)
            } else {
      ...
      function scriptVerify (obj) {
        return new Promise(function (resolve, reject) {
          ...
          for (let i = 0; i < document.querySelectorAll('script').length; i++) {
            if (document.querySelectorAll('script')[i].src.indexOf(`${obj['component']}-bundle}`) !== -1) {
              verify = true
            }
          }
          if (verify === true) {
            ...
            if (!obj['this'].shadowRoot) {
              obj['this'].appendChild(script)
            } else {
              obj['this']['shadowRoot'].insertBefore(script, obj['this']['shadowRoot'].firstChild)
            }
