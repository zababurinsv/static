customElements.define('audio-chain',
    ...
    constructor () {
      ...
      function oscilloscope (obj) {
        return new Promise(function (resolve, reject) {
          osc['lifecycle'] = []
          osc['type-waves'] = waves

          console.log('!!!!!!!!3333!!!!!!!', obj[`template-shadow`][3])
          // obj[`template-light`]
          for (let state in obj['state']) {
            for (let key in obj[`template-${obj['state'][state]}`]) {
              if (!obj[`template-${obj['state'][state]}`][key].querySelector('.frequency input')) {
              } else {
                osc['frequencyInput'] = obj[`template-${obj['state'][state]}`][key].querySelector('.frequency input')
              }
              if (!obj[`template-${obj['state'][state]}`][key].querySelector('.frequency span')) {
              } else {
                osc['frequencySpan'] = obj[`template-${obj['state'][state]}`][key].querySelector('.frequency span')
              }
              if (!obj[`template-${obj['state'][state]}`][key].querySelector('.note span')) {
              } else {
                osc['noteSpan'] = obj[`template-${obj['state'][state]}`][key].querySelector('.frequency span')
              }
            }
          }
          ...
          for (let waves in osc['type-waves']) {
            let option = document.createElement('option')
          }
          osc['lifecycle']['typeSelect'] = obj['this'].querySelector('.type')
          osc['lifecycle']['created'] = function () {
          }
      ...
      async function listener (obj) {
        ...
        // obj['this'].frequencyInput.addEventListener('change', function () {
        // var value = parseFloat(this.value)
        // self.frequency = value
        // }, false)
        // obj['this']['typeSelect'].addEventListener('change', function (e) {
        //   obj['this'].type = obj['this'].typeSelect.value
        // }, false)
