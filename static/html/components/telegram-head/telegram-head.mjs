...
customElements.define('telegram-head',
    class extends HTMLElement {
      ...
      constructor () {
        ...
        async function modules (obj) {
          ...
          document.addEventListener('messageToLeft',async (e)=>{
            socket['socket'].send(JSON.stringify({
              type:'sendMessage',
              chat_id:obj['this']['shadowRoot'].querySelector('.private').innerText ,
              text:e['detail']['data'] }))
          })


          // document.addEventListener('authorizationStateReady', async (e)=>{


            // let name = `${e['detail']['data']['first_name']} ${e['detail']['data']['last_name']}`
            // obj['this']['shadowRoot'].querySelector('#avatar')
            // obj['this']['shadowRoot'].querySelector('#name').innerHTML = ''
            // obj['this']['shadowRoot'].querySelector('#name').innerHTML = name
            // obj['this']['shadowRoot'].querySelector('.info').innerHTML = ''
            // obj['this']['shadowRoot'].querySelector('.info').innerHTML = e['detail']['data']['id']
            // console.assert(false, e['detail']['data'])

          // })

          document.addEventListener('PrivateChat',async (e)=>{
              obj['this']['shadowRoot'].querySelector('#name').innerText = ''
              obj['this']['shadowRoot'].querySelector('#name').innerText = e['detail']['title']
              obj['this']['shadowRoot'].querySelector('.info').innerText = ''
              obj['this']['shadowRoot'].querySelector('.info').innerText = e['detail']['id']
