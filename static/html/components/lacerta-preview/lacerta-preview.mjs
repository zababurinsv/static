import MutationObserver from '/static/html/components/component_modules/MutationObserver/MutationObserver.mjs'
import styleTemplate from '/static/html/components/component_modules/styleTemplate/styleTemplate.mjs'
import store from '/static/html/components/component_modules/staticProperty/staticProperty.mjs'

customElements.define('lacerta-preview',
    class extends HTMLElement {
        constructor () {
            ...
            function editorObject (obj) {
                return new Promise(function (resolve, reject) {
                    let editorObject = {}
                    editorObject['largeImg']
                    editorObject['thumbs']
                })
            }
            async function modules (obj) {
               await store({
                    input:'lacerta-preview',
                    this:obj['this'],
                    obj: obj,
                    type:'obj'
                }, 'set', 'type')
                // console.assert(false, document['body'].shadowRoot.querySelector('nav'))
                if(document['body'].shadowRoot.querySelector('nav').className === 'active'){
                    styleTemplate({
                        input:'lacerta-preview',
                        type:'admin',
                        data: obj['this']
                ...
                // MutationObserver({
                //     input:'index',
                //     this: document['body'].querySelector('nav'),
                //     type:'nav'
                // },'set', 'type')
            }
        }
