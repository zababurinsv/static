...
customElements.define('telegram-page',
    class extends HTMLElement {
      ...
      constructor () {
        ...
        async function modules (obj) {
          // console.assert(false, await config('telegram', 'wss'))
          let socket = new (await webSocket({name:'telegram-login'}))['class'](await config('telegram', 'localWss'))
          document.addEventListener('authorizationStateReady', async (object)=>{
            socket['socket'].send(JSON.stringify({type:'getMe'}));
            let state = await store({
              input:'state',
              type:'all'
            }, 'get', 'type')
            obj['this']['shadowRoot'].querySelector('#chat').style.display = 'flex'
            obj['this']['shadowRoot'].querySelector('#login').style.display = 'none'
              let nav = document['body']['shadowRoot'].querySelectorAll('nav')
              for(let i =0; i < nav.length; i++){
                nav[i].style.display = 'flex'
              }
            state['lacerta-preview'][0]['this']['shadowRoot'].querySelector('.rightGraphic').style.right = '-4vw'
            state['lacerta-preview'][0]['this']['shadowRoot'].querySelector('.leftGraphic').style.left = '-4vw'
          })
          document.addEventListener('authorizationStateClosed', async (object)=>{
            let state = await store({
              input:'state',
              type:'all'
            }, 'get', 'type')
            obj['this']['shadowRoot'].querySelector('#chat').style.display = 'none'
            obj['this']['shadowRoot'].querySelector('#login').style.display = 'flex'
            let nav = document['body']['shadowRoot'].querySelectorAll('nav')
            for(let i =0; i < nav.length; i++){
              nav[i].style.display = 'none'
            }
            state['lacerta-preview'][0]['this']['shadowRoot'].querySelector('.rightGraphic').style.right = '0vw'
            state['lacerta-preview'][0]['this']['shadowRoot'].querySelector('.leftGraphic').style.left = '0vw'
          })
