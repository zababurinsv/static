import message from '/static/html/components/component_modules/message/message.mjs'
...
customElements.define('telegram-message',
    class extends HTMLElement {
      ...
      constructor () {
        ...
        async function modules (obj) {
          ...
          let panel = new (await message({name:'telegram-message'}))['class']
          let textarea = obj['this']['shadowRoot'].querySelector('.message');

          textarea.addEventListener('input', autosize);


          async function autosize(e){
            if(e.target.value.length === 0){
              panel['null'](obj,'')
              obj['this']['shadowRoot'].querySelector('#command').style.display = 'flex'
            }else{
              obj['this']['shadowRoot'].querySelector('#command').style.display = 'none'
              await panel['message'](obj,'')

              obj['this']['shadowRoot'].querySelector('img.message').addEventListener('click', async (e)=>{

                let messageToLeft = new CustomEvent('messageToLeft', {
                  detail: {
                    data: obj['this']['shadowRoot'].querySelector('textarea.message').value
                  }
                })
                document.dispatchEvent(messageToLeft)
                obj['this']['shadowRoot'].querySelector('textarea.message').value = ''

              })
            }
            // console.assert(false, e.target.value)
            let el = this;
            setTimeout(function(){
              el.style.cssText = 'height:auto; padding:0';
              // for box-sizing other than "content-box" use:
              // el.style.cssText = '-moz-box-sizing:content-box';
              let h = el.scrollHeight +  'px'
              el.style.cssText = 'height:' + h;
              // obj['this']['shadowRoot'].querySelector('#command').style.cssText = 'height:' + el.scrollHeight +  'px'
              obj['this']['shadowRoot'].querySelector('#attach').style.cssText = 'height:' + h
            },0);
          }

          // fetch('/static/images/JSDesign/WebDesign_v0.1/previews/preview.png')
          //     .then(function (response) {
          //       if (response.ok) {
